...
| Optional (Explicit)           | Field is optional — the form says "optional"/"if applicable"/"if known", or nothing marks it required and it is not business-necessary | Set `required: {"value": false}` explicitly (omission inherits `required: true` from the registry on generic primitives). Never write "(optional)" into the label or hint — the renderer appends the suffix from this value |
