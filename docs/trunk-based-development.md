...
_How we develop with `main` as the trunk. Pairs with the "Fix promotion &
...
and the migration mechanics in [plans/go-live-handoff.md](plans/go-live-handoff.md)._
...
DevOps, Humble & Farley's _Continuous Delivery_, Fowler); citations in §7.
Where a recommendation applies a general principle to _our_ nx/Amplify/ECS
...
> the **cutover** (making `main` the default branch) are the _target_ and are
...
merged back via a PR that must pass **CI** (no review required _for now_ — see
...
| **Test**             | `nx affected -t test` — unit tests (+ coverage thresholds, enforced by Vitest) |
...
**Environments are deploy targets, not workspaces.** Code only ever flows _into_
...
Branches exist only for review + CI before landing — _"not artifact creation or
publication."_ Target **hours**, cap **~2 days**; DORA's high-performer signal
...
DORA core CI element: _"when the build breaks, fixing it should take priority
over any other work."_ A red `main` (or a red `sandbox` deploy that's blocking
...
- `main` reset to current code; it's the trunk and the repository's default branch.
- **Merge gate enforced** — "Main CI Required" ruleset (PR + the checks in §2,
  no review).
- **Sequential fan-out wired** — `main` → `sandbox` (auto) → `staging`
  (on sandbox-green) → `prod` (manual/windowed), from workflows on `main`.
- README, [CLAUDE.md](../CLAUDE.md), this playbook, and the diagram reflect the model.
...
- **DORA / Google State of DevOps** (dora.dev) — TBD definition (≤3 active
  ...
  "stop the line"; _effective suites only pass releasable code_. (Correlational,
  not proven-causal.)
- **Humble & Farley, _Continuous Delivery_** — deployment pipeline; commit stage
  (<5 min ideal / <10 max; mostly unit + a small selection of others);
  no-promotion-on-failure.
- **Martin Fowler** — _Continuous Integration_, _Feature Toggles_, _Practical
  Test Pyramid_, the "ten-minute build."
- **Google** — _Software Engineering at Google_ (ch. 9 review, ch. 23 CI/CD).

_The nx-affected / Amplify-ECS / sequential-deploy mappings are our synthesis of
these principles onto our stack, not direct quotes. The "no review for now"
choice is a deliberate, temporary deviation from the canonical model._
