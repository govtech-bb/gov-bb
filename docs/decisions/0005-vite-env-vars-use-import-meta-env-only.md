...
- The test runner must support the pattern. `apps/forms` now runs on Vitest, which reads `import.meta.env` natively — tests drive `VITE_*` values per-case with `vi.stubEnv` (no AST transformer needed). _(Historically this app used ts-jest in CommonJS mode with `ts-jest-mock-import-meta`; that note is retained for any future Vite app still on Jest, which would need the equivalent transformer.)_
