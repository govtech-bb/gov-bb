...
- A future request to "let operators test-submit a preview" must not be met by
  ...
  - **Update (ADR 0065, narrowed by ADR 0066):** this still holds for **DB-only
    builder drafts**. Non-public *published file* recipes (the #1682
    `bypassVisibility` case) are a different animal and are submittable on
    environments that name them in `PREVIEW_SUBMISSION_FORM_IDS`; that path
    never reaches a DB draft.
