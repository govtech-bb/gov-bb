...
describe("createPublishClient", () => {
  ...
  describe("listOpenPRHeads", () => {
    ...
    it("fails closed when the five-page inventory limit is exhausted", async () => {
      mockFetch(() =>
        Promise.resolve({
          ok: true,
          json: () =>
      ...
      const gh = createPublishClient(REPO);
      await expect(gh.listOpenPRHeads("tok", "dev")).rejects.toThrow(
        /inventory is incomplete/,
      );
    });
  });
  describe("findOpenPRByHeadRef", () => {
    it("returns the open PR whose head ref matches the predicate", async () => {
      mockFetch(() =>
        Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve([
              { number: 1, html_url: "https://pr/1", head: { ref: "other" } },
              { number: 2, html_url: "https://pr/2", head: { ref: "target" } },
            ]),
        }),
      );
      const gh = createPublishClient(REPO);
      const pr = await gh.findOpenPRByHeadRef(
        "tok",
        "dev",
        (ref) => ref === "target",
      );
    ...
    it("returns null when no open PR matches", async () => {
      ...
      const gh = createPublishClient(REPO);
      const pr = await gh.findOpenPRByHeadRef("tok", "dev", () => false);
      expect(pr).toBeNull();
    });

    it("returns the highest-numbered match when several PRs match", async () => {
      // Shouldn't happen in practice (one head ref -> one open PR), but if
      // someone opens a duplicate by hand, the most recent (highest number)
      // wins so the result stays deterministic (#2390).
      mockFetch(() =>
        Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve([
              { number: 5, html_url: "https://pr/5", head: { ref: "dup" } },
              { number: 12, html_url: "https://pr/12", head: { ref: "dup" } },
              { number: 8, html_url: "https://pr/8", head: { ref: "dup" } },
            ]),
        }),
      );
      const gh = createPublishClient(REPO);
    ...
    it("passes each PR's head ref string to the predicate", async () => {
      mockFetch(() =>
        Promise.resolve({
          ok: true,
          json: () =>
      ...
      const gh = createPublishClient(REPO);
      const seen: string[] = [];
      await gh.findOpenPRByHeadRef("tok", "dev", (ref) => {
        seen.push(ref);
        return false;
      });
  ...
  describe("commentOnPR", () => {
    it("POSTs to the issue-comments endpoint with the right headers and body", async () => {
      const fetchMock = mockFetch(() =>
        Promise.resolve({ ok: true, json: () => Promise.resolve({}) }),
      );
      const gh = createPublishClient(REPO);
      await gh.commentOnPR("tok", 42, "hello there");
      const [url, init] = fetchMock.mock.calls[0];
      expect(url).toBe(
    ...
    it("throws on a non-ok response", async () => {
      ...
      const gh = createPublishClient(REPO);
      await expect(gh.commentOnPR("tok", 42, "hello")).rejects.toThrow(
        /Failed to comment/,
      );
    });
  });
