...
export type BreadcrumbsSize = keyof typeof breadcrumbsStyles.size;
...
export function breadcrumbsVariants({
  ...
  return cn(
    ...
    breadcrumbsStyles.size[size] ?? breadcrumbsStyles.size["base"],
  );
}
...
const Link = ({
  ...
  return (
    <LinkComponent
      ...
      className="flex shrink-0 items-center gap-1 whitespace-nowrap text-ui-subtle no-underline"
    >
...
function Separator() {
  return (
    <span
      className="flex shrink-0 items-center text-ui-inactive"
      aria-hidden="true"
    >
...
function MobileEllipsis() {
  return (
    <span className="flex shrink-0 items-center text-ui-subtle" aria-hidden>
      ...
    </span>
  );
}
function Clipboard({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {isCopied ? (
        <CheckIcon weight="bold" className="text-ui-success" />
      ) : (
...
export interface BreadcrumbsProps
  extends PropsWithChildren, BreadcrumbsVariantsProps {
...
function getMobileBreadcrumbChildren(children: ReactNode[]): ReactNode[] {
  ...
  const trailingItems: ReactNode[] = [
    <MobileEllipsis key="ui-breadcrumb-mobile-ellipsis" />,
    <Separator key="ui-breadcrumb-mobile-separator-leading" />,
    cloneElement(parentItem, { key: "ui-breadcrumb-mobile-parent" }),
    <Separator key="ui-breadcrumb-mobile-separator-trailing" />,
    cloneElement(currentItem, { key: "ui-breadcrumb-mobile-current" }),
  ];
