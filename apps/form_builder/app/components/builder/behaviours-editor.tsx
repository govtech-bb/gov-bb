...
function FieldArrayMiniature({
  ...
  return (
    <div
      className="mt-1 w-full rounded-lg border border-dashed border-ui-line bg-ui-surface-2 px-3 py-2.5"
      aria-hidden="true"
    >
      <div className="mb-1.5 text-[11px] font-semibold tracking-[0.04em] text-ui-subtle uppercase">
        The applicant sees
      </div>
      <div className="mb-1 text-[13px] font-medium text-ui-default">
        {fieldLabel || "Field label"}
      </div>
      {Array.from({ length: boxes }).map((_, i) => (
        <div
          key={i}
          className="mb-1 h-5.5 max-w-65 rounded-md border border-ui-inactive bg-ui-base"
          data-testid="fa-miniature-box"
        />
      ))}
      {min > boxes && (
        <div className="mb-1 text-[12px] text-ui-subtle">
          …and {min - boxes} more
        </div>
      )}
      {min < max && (
        <div className="text-[13px] text-ui-default underline">
          + {linkText}
        </div>
      )}
    </div>
...
function InValueInput({
  ...
  return (
    <div className="mb-3.5 flex flex-col gap-1.25 [&_input]:box-border [&_input]:w-full [&_textarea]:box-border [&_textarea]:w-full [&_label]:text-[13px] [&_label]:font-medium [&_label]:text-ui-brand-hover [[data-field-row]>&]:w-full">
      ...
      />
      <small className="mt-1 block text-[12px] leading-[1.3] text-ui-subtle">
            ...
            {descriptor?.params.map((param) => {
              ...
              if (param.kind === "fieldRefArray") {
                ...
                return (
                  <fieldset
                    key={param.name}
                    className="mb-3.5 flex flex-col gap-1.25 [&_input]:box-border [&_input]:w-full [&_textarea]:box-border [&_textarea]:w-full [&_label]:text-[13px] [&_label]:font-medium [&_label]:text-ui-brand-hover [[data-field-row]>&]:w-full min-w-0"
                  >
                    <legend className="text-[13px] font-medium text-ui-brand-hover">
                      {param.label}
                    </legend>
                    {options.map((f) => {
