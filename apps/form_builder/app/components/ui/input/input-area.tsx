...
function useTextareaAutoResize({
  ...
  const resize = useCallback(() => {
    ...
    if (!enabled || !textarea || typeof window === "undefined") return;
...
export const InputArea = React.forwardRef<HTMLTextAreaElement, InputAreaProps>(
  (props, ref) => {
    ...
    const textareaClassName = cn(
      ...
      autoResize &&
        "field-sizing-content w-full resize-none scroll-pb-2 [scrollbar-color:var(--ui-hairline)_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar]:bg-transparent [&::-webkit-scrollbar-corner]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-ui-hairline [&::-webkit-scrollbar-track]:my-2",
      className,
    );
    // An element render lets Base UI merge its validation handlers and refs.
