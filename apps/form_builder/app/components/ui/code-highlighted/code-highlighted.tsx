...
export function CodeHighlighted({
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      console.error("[CodeHighlighted] Failed to copy to clipboard:", err);
    }
  }, [code]);
  ...
  const containerClasses = cn(
    "group relative m-0 w-full min-w-0 rounded-md border border-ui-tint bg-ui-base p-0",
  ...
  const lineNumbers =
    showLineNumbers && !isSingleLine ? (
      <div
        className="ui-line-numbers shrink-0 py-4 pr-4 text-right font-mono text-sm opacity-40 select-none"
        aria-hidden="true"
      >
  ...
  useEffect(() => {
    if (error) {
      console.error("[CodeHighlighted] Shiki initialization error:", error);
    }
  }, [error]);
    ...
    return (
      <div className={containerClasses}>
        {lineNumbers && (
          <div className="flex">
            {lineNumbers}
            <pre className="!m-0 min-w-0 flex-1 ui-scroll-native overflow-x-auto !p-4 font-mono text-sm leading-relaxed text-ui-subtle">
    ...
    <div className={containerClasses}>
      {lineNumbers && (
        <div className="flex w-full">
          {lineNumbers}
          <div className="min-w-0 flex-1 ui-scroll-native overflow-x-auto">
            <div
              className="ui-shiki [&_code]:!m-0 [&_code]:!border-0 [&_code]:!bg-transparent [&_code]:!p-0 [&>pre]:!m-0 [&>pre]:!rounded-none [&>pre]:!border-0 [&>pre]:!bg-transparent [&>pre]:!p-4 [&>pre]:font-mono [&>pre]:text-sm [&>pre]:leading-relaxed"
      ...
      {!lineNumbers && (
        <div className="ui-scroll-native overflow-x-auto">
          <div
            className="ui-shiki [&_code]:!m-0 [&_code]:!border-0 [&_code]:!bg-transparent [&_code]:!p-0 [&>pre]:!m-0 [&>pre]:!rounded-none [&>pre]:!border-0 [&>pre]:!bg-transparent [&>pre]:!p-4 [&>pre]:font-mono [&>pre]:text-sm [&>pre]:leading-relaxed"
            dangerouslySetInnerHTML={{
              __html: processHighlightedHtml(html, highlightLines),
