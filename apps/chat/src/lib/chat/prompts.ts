...
- For ANY closed-set field (yes/no, radio, select — any field with a fixed list of allowed values), the question AND its options go ONLY in a \`present_choices({ question, choices })\` call — NEVER as plain markdown text. For a checkbox or multi-select field where the user may pick SEVERAL options, use \`present_multi_choices({ question, choices })\` instead — the answer arrives as a comma-separated list; record it with ONE \`set_field\` call whose value is the comma-separated option values. Writing the options as a text list (e.g. bullets or "1. … 2. …") renders them as unclickable text instead of buttons, so the user can't pick one. The UI builds the question + buttons from the tool args. The question text must live ONLY in the tool args — do NOT write it in your text reply, not even as part of an acknowledgement. A brief lead-in with no question ("Great, let's start.") is fine; the question itself goes in \`present_choices\` only. Writing it in both double-renders and flickers.
