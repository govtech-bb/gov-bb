...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      "process.env.LLM_PROVIDER": JSON.stringify(env.LLM_PROVIDER || process.env.LLM_PROVIDER || "anthropic"),
      "process.env.LLM_MODEL": JSON.stringify(env.LLM_MODEL || process.env.LLM_MODEL || "claude-haiku-4-5"),
      "process.env.BEDROCK_REGION": JSON.stringify(env.BEDROCK_REGION || process.env.BEDROCK_REGION || "ca-central-1"),
      "process.env.FORM_API_URL": JSON.stringify(env.FORM_API_URL || process.env.FORM_API_URL || ""),
      "process.env.DATABASE_URL": JSON.stringify(env.DATABASE_URL || process.env.DATABASE_URL || ""),
      "process.env.DB_SSL": JSON.stringify(env.DB_SSL || process.env.DB_SSL || ""),
      "process.env.RAG_URL": JSON.stringify(env.RAG_URL || process.env.RAG_URL || ""),
