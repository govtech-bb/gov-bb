...
const baseSchema = z
  .object({
    ...
    // Admin endpoints stopgap auth (#286): shared secret for the AdminTokenGuard
    // (x-admin-token header). Defaults to "" so an unset value is "" (dev
    // passthrough); the production-required guard is in superRefine below, where
    // an empty "" is treated as missing.
...
export const envValidationSchema = baseSchema
  .superRefine((env, ctx) => {
    ...
    // ADMIN_API_TOKEN required in production (#286 stopgap admin guard).
    if (env.NODE_ENV === "production" && !env.ADMIN_API_TOKEN) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["ADMIN_API_TOKEN"],
        message: '"ADMIN_API_TOKEN" is required when NODE_ENV=production',
      });
    }
