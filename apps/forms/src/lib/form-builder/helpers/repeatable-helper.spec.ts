...
describe("setupRepeatSteps", () => {
  ...
  it("source step holds ONLY the shared fields; per-instance fields are not duplicated onto it (#1257)", () => {
    ...
    // The config carries sharedData keyed by the shared fieldId — this is the
    // signal the submit-time fold uses to treat the base step as a shared-values
    // page (not an instance) and exclude it (#1257).
    expect(
      Object.keys(repeatSettings["childDetails"].sharedData ?? {}),
    ).toEqual(["childSchool"]);
  });
