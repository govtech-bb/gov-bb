/**
 * Every query lives here, behind the same operations `packages/spike-db`'s
 ...
 * spike it ran only in the browser, which made the nine rules advisory —
 * anyone could POST whatever they liked. A rule that is not enforced where
 * the data lands is documentation, not a rule.
 */
