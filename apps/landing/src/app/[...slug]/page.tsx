...
function resolveData(slugSegments: string[]): LoaderData | null {
  const splat = slugSegments.join('/')

  if (slugSegments.length === 1) {
    const cat = CATEGORY_BY_SLUG[slugSegments[0]]
    if (cat) {
  ...
  if (slugSegments.length === 2) {
    const cat = CATEGORY_BY_SLUG[slugSegments[0]]
    const sub = cat ? getSubcategory(cat.slug, slugSegments[1]) : undefined
    if (cat && sub) {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (data.kind === 'subcategory') {
    return {
      title: `${data.subcategory.title} | ${data.category.title}`,
    }
  }
...
export function generateStaticParams() {
  ...
  // Category pages
  for (const cat of Object.values(CATEGORY_BY_SLUG)) {
