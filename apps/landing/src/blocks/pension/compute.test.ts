...
describe("calculatePension", () => {
  ...
  it("flags service warning below 10 years (120 months)", () => {
    expect(calculatePension({ months: 119, salary: 50_000 }).serviceWarning).toBe(true);
    expect(calculatePension({ months: SERVICE_WARNING_MONTHS, salary: 50_000 }).serviceWarning).toBe(false);
  });
