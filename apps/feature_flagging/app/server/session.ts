...
// even though their `beforeLoad` only runs server-side. Crypto-using
// counterparts (encrypt/decrypt, getSession, setSession, safeEqual) live in
// `./session-cipher.server.ts`, which the import-protection plugin keeps out of
// client bundles.

export const SESSION_COOKIE_NAME = "ff_session";
const OAUTH_STATE_COOKIE_NAME = "ff_oauth_state";
...
/** Extract the value of `ff_session` from a `Cookie` header. Returns null if absent. */
export function parseSessionCookie(header: string | null): string | null {
  return parseNamedCookie(header, SESSION_COOKIE_NAME);
}

/** Extract the value of `ff_oauth_state` from a `Cookie` header. Returns null if absent. */
