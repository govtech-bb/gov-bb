 ...
 *   - ARCHIVE_DRAFTS_TOKEN: shared secret for the admin endpoint, sent as the
 *     `x-admin-token` header. MUST equal the API's ADMIN_API_TOKEN (#286).
 *
 * Behavior:
  ...
  for (const { formId } of entries) {
    ...
    try {
      const res = await fetchFn(url, {
        method: "POST",
        headers: {
          // AdminTokenGuard (#286) reads the shared secret from `x-admin-token`,
          // NOT Authorization/Bearer. ARCHIVE_DRAFTS_TOKEN must hold the same
          // value as the API's ADMIN_API_TOKEN or this 403s.
          "x-admin-token": token,
